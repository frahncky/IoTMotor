...
void publishTelemetry(bool includeVibration, bool includeTemperature, const char* requestId) {
  if (!includeVibration && !includeTemperature) {
    return;
  }
...
void handleTelemetryRequest(const JsonDocument& requestDoc) {
  ...
  publishTelemetry(includeVibration, includeTemperature, requestId);
}
...
void connectMqtt() {
  ...
  while (!mqttClient.connected()) {
    String clientId = String("esp32_env_") + String(DEVICE_ID);
