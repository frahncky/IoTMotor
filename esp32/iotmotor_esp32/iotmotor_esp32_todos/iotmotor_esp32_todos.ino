...
static const char* DEVICE_ID = "esp32-03";
...
static float simulatedTemp = 29.0f;
...
void applyCommand(const char* command, const char* mode) {
  if (strcmp(command, "start") == 0) {
    ...
    publishTelemetry(true, true, true, true, "");
    return;
  }
...
TelemetrySample generateTelemetrySample() {
  ...
  sample.voltage = clampValue(
    220.0f + voltageWave - voltageDrop + randomRange(-0.5f, 0.5f),
    210.0f,
    230.0f
  );

  if (motorEnabled) {
    sample.current = clampValue(
      5.5f + sinf(simPhase * 1.7f) * 1.2f + randomRange(-0.25f, 0.25f),
      3.8f,
      8.5f
    );
    sample.vibration = clampValue(
      0.38f + fabsf(sinf(simPhase * 2.4f)) * 0.25f + randomRange(-0.05f, 0.05f),
      0.15f,
      0.95f
    );

    if (random(0, 100) < 3) {
      sample.vibration = clampValue(sample.vibration + randomRange(0.15f, 0.35f), 0.0f, 1.2f);
    }
  } else {
    sample.current = clampValue(0.12f + randomRange(-0.03f, 0.03f), 0.02f, 0.25f);
    sample.vibration = clampValue(0.03f + randomRange(-0.02f, 0.02f), 0.0f, 0.10f);
  }

  float targetTemp = motorEnabled ? 47.0f : 29.0f;
  simulatedTemp += (targetTemp - simulatedTemp) * 0.08f + randomRange(-0.08f, 0.08f);
  sample.temperature = clampValue(simulatedTemp, 24.0f, 70.0f);
...
void publishTelemetry(
  bool includeVoltage,
  bool includeCurrent,
  bool includeVibration,
  bool includeTemperature,
  const char* requestId
) {
  if (!includeVoltage && !includeCurrent && !includeVibration && !includeTemperature) {
    return;
  }
...
void handleTelemetryRequest(const JsonDocument& requestDoc) {
  ...
  bool includeVibration = isFieldRequested(fieldsVariant, "vibration");
  bool includeTemperature = isFieldRequested(fieldsVariant, "temperature");
...
void connectMqtt() {
  ...
  while (!mqttClient.connected()) {
    String clientId = String("esp32_all_") + String(DEVICE_ID);
