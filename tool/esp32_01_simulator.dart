...
const String _defaultDeviceId = 'esp32-01';
const int _defaultIntervalMs = 1000;
const Set<String> _supportedFields = <String>{'voltage', 'current'};
...
Future<void> main(List<String> args) async {
  ...
  Map<String, dynamic>? buildTelemetryPayload({
    ...
    final bool hasMetrics =
        payload.containsKey('voltage') || payload.containsKey('current');
    if (!hasMetrics) {
      return null;
    }
...
void _printUsage() {
  stdout.writeln('Simulador MQTT esp32-01 (voltage + current)');
  stdout.writeln('');
  stdout.writeln('Uso:');
  stdout.writeln('  dart run tool/esp32_01_simulator.dart [opcoes]');
  stdout.writeln('');
